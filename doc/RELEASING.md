...
make version-bump o=0.1.9 n=0.1.9
```
...
2. Generate a changelog entry from git commits since v0.1.9
...
- `yamlstar-0.1.9-linux-x64.tar.xz`
- `yamlstar-0.1.9-macos-arm64.tar.xz`
- `libyamlstar-0.1.9-linux-x64.tar.xz`
- `libyamlstar-0.1.9-macos-arm64.tar.xz`
...
make release-bindings VERSION=0.1.9
```

The `release-bindings` target:
1. Verifies GitHub Release `v0.1.9` exists
...
```
ERROR: VERSION=0.1.9 does not match Meta file version: 0.1.9
```
...
```
ERROR: Missing yamlstar-0.1.9-linux-x64.tar.xz
```
...
```
ERROR: Tag v0.1.9 already exists
```
