...
make version-bump o=0.1.17 n=0.1.17
```
...
2. Generate a changelog entry from git commits since v0.1.17
...
5. Enter the version (e.g., `0.1.17`)
...
make release-bindings VERSION=0.1.17
```

The `release-bindings` target:
1. Verifies GitHub Release `v0.1.17` exists
...
```
ERROR: VERSION=0.1.17 does not match Meta file version: 0.1.17
```
...
```
ERROR: Missing yamlstar-0.1.17-linux-x64.tar.xz
```
...
```
ERROR: Tag v0.1.17 already exists
```
