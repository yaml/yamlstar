...
make version-bump o=0.1.21 n=0.1.21
```
...
2. Generate a changelog entry from git commits since v0.1.21
...
5. Enter the version (e.g., `0.1.21`)
...
- `yamlstar-0.1.21-linux-x64.tar.xz`
- `yamlstar-0.1.21-linux-aarch64.tar.xz`
- `yamlstar-0.1.21-macos-arm64.tar.xz`
- `yamlstar-0.1.21-macos-x64.tar.xz`
- `yamlstar-0.1.21-freebsd-x64.tar.xz`
- `yamlstar-0.1.21-windows-x64.zip`
- `yamlstar-0.1.21-windows-arm64.zip`
- `yamlstar-0.1.21-wasm-p1.tar.xz`
- `libyamlstar-0.1.21-linux-x64.tar.xz`
- `libyamlstar-0.1.21-linux-aarch64.tar.xz`
- `libyamlstar-0.1.21-macos-arm64.tar.xz`
- `libyamlstar-0.1.21-macos-x64.tar.xz`
- `libyamlstar-0.1.21-freebsd-x64.tar.xz`
- `libyamlstar-0.1.21-windows-x64.zip`
- `libyamlstar-0.1.21-windows-arm64.zip`
...
make release-bindings VERSION=0.1.21
```

The `release-bindings` target:
1. Verifies GitHub Release `v0.1.21` exists
...
make check-version VERSION=0.1.21

# Build shared library for current platform only
make release-lib VERSION=0.1.21

# Build CLI for current platform only
make release-cli VERSION=0.1.21

# Create and push git tag (normally done by workflow)
make release-tag VERSION=0.1.21

# Create GitHub release (normally done by workflow)
make release-github VERSION=0.1.21

# Check that GitHub release exists with all assets
make check-release VERSION=0.1.21

# Publish Homebrew formulas
make release-homebrew v=0.1.21

# Publish Python only
make release-python VERSION=0.1.21
...
```
ERROR: VERSION=0.1.21 does not match Meta file version: 0.1.21
```
...
```
ERROR: Missing yamlstar-0.1.21-linux-x64.tar.xz
```
...
```
ERROR: Tag v0.1.21 already exists
```
