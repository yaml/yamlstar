...
make version-bump o=0.1.15 n=0.1.15
```
...
2. Generate a changelog entry from git commits since v0.1.15
...
5. Enter the version (e.g., `0.1.15`)
...
- `yamlstar-0.1.15-linux-x64.tar.xz`
- `yamlstar-0.1.15-macos-arm64.tar.xz`
- `libyamlstar-0.1.15-linux-x64.tar.xz`
- `libyamlstar-0.1.15-macos-arm64.tar.xz`
...
make release-bindings VERSION=0.1.15
```

The `release-bindings` target:
1. Verifies GitHub Release `v0.1.15` exists
...
make check-version VERSION=0.1.15

# Build shared library for current platform only
make release-lib VERSION=0.1.15

# Build CLI for current platform only
make release-cli VERSION=0.1.15

# Create and push git tag (normally done by workflow)
make release-tag VERSION=0.1.15

# Create GitHub release (normally done by workflow)
make release-github VERSION=0.1.15

# Check that GitHub release exists with all assets
make check-release VERSION=0.1.15

# Publish Homebrew formulas
make release-homebrew n=0.1.15

# Publish Python only
make release-python VERSION=0.1.15
...
```
ERROR: VERSION=0.1.15 does not match Meta file version: 0.1.15
```
...
```
ERROR: Missing yamlstar-0.1.15-linux-x64.tar.xz
```
...
```
ERROR: Tag v0.1.15 already exists
```
