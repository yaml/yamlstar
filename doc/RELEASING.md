...
make version-bump o=0.1.5 n=0.1.5
```
...
2. Generate a changelog entry from git commits since v0.1.5
...
- `yamlstar-0.1.5-linux-x64.tar.xz`
- `yamlstar-0.1.5-macos-arm64.tar.xz`
- `libyamlstar-0.1.5-linux-x64.tar.xz`
- `libyamlstar-0.1.5-macos-arm64.tar.xz`
...
make release-bindings VERSION=0.1.5
```

The `release-bindings` target:
1. Verifies GitHub Release `v0.1.5` exists
...
make check-version VERSION=0.1.5

# Build shared library for current platform only
make release-lib VERSION=0.1.5

# Build CLI for current platform only
make release-cli VERSION=0.1.5
...
```
ERROR: VERSION=0.1.5 does not match Meta file version: 0.1.5
```
...
```
ERROR: Missing yamlstar-0.1.5-linux-x64.tar.xz
```
...
```
ERROR: Tag v0.1.5 already exists
```
